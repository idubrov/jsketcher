import App2D, { STORAGE_PREFIX } from './sketcher/main2d';
import { Layer, Styles } from './sketcher/canvas';
import { Window, List, bindOpening, createActionsWinBuilder } from './ui';
import { utils, Constraints } from './sketcher/parametric';
...
function start() {
  ...
  if (sketchId == STORAGE_PREFIX + '__sample2D__') {
  ...
  var actionsWin = new Window($('#actions'), app.winManager);

  bindOpening( $('#showActions'), actionsWin );
  var addAction = createActionsWinBuilder(actionsWin);
  ...
  var constrList = new List('constrs', {
    items : function() {
      var theItems = [];
    ...
    click : function(item) {
      ...
      for (var f in c.SettableFields) {
        ...
        value = utils.askNumber(intro, typeof(value) == "number" ? value.toFixed(4) : value, prompt, pm.constantResolver);
        if (value != null) {
          c[f] = value;
        }
      }
  ...
  function configureConstraintsFilter() {
    ...
    var constraintsFilterBtn = App2D.faBtn("filter");
    constraintsFilterBtn.css({'float': 'right', 'margin-right': '10px', cursor: 'pointer'});
    constraintsCaption.append(constraintsFilterBtn);
    var constraintsFilterWin = new Window($('#constrFilter'), app.winManager);
    bindOpening(constraintsFilterBtn, constraintsFilterWin);
...
export default function sketcherAppInit() {
  start();
}
