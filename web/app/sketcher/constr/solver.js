...
/** @constructor */
parametric.Param = function(id, value, readOnly) {
  this.reset(value);
};

parametric.Param.prototype.reset = function(value) {
  this.set(value);
  this.j = -1;
};

parametric.Param.prototype.set = function(value) {
  this.value = value;
};

parametric.Param.prototype.get = function() {
  return this.value;
};

parametric.Param.prototype.nop = function() {};

/** @constructor */
parametric.System = function(constraints) {
...
parametric.System.prototype.makeJacobian = function() {
  var jacobi = [];
  var i;
  var j;
...
parametric.System.prototype.fillJacobian = function(jacobi) {
  var j;
...
parametric.System.prototype.calcResidual = function(r) {
...
parametric.System.prototype.calcGrad_ = function(out) {
  var i;
...
parametric.System.prototype.calcGrad = function(out) {
  var i;
  for (i = 0; i < out.length || i < this.params.length;  ++i) {
    out[i] = 0;
  }
...
parametric.System.prototype.fillParams = function(out) {
  for (var p = 0; p < this.params.length; p++) {
    out[p] = this.params[p].get();
  }
};

parametric.System.prototype.getParams = function() {
  var out = [];
  this.fillParams(out);
  return out;
};

parametric.System.prototype.setParams = function(point) {
  for (var p = 0; p < this.params.length; p++) {
    this.params[p].set(point[p]);
  }
};

parametric.System.prototype.error = function() {
  var error = 0;
  for (var i=0; i < this.constraints.length; i++) {
...
parametric.System.prototype.errorSquare = function() {
  var error = 0;
  for (var i=0; i < this.constraints.length; i++) {
...
parametric.System.prototype.getValues = function() {
  var values = [];
  for (var i=0; i < this.constraints.length; i++) {
...
parametric.wrapAux = function(constrs, locked) {
  ...
  for (var i = 0; i < constrs.length; i++) {
    ...
    if (needWrap) {
      var wrapper = new constraints.ConstantWrapper(c, mask);
      constrs[i] = wrapper;
    }
  }
};

parametric.lock2Equals2 = function(constrs, locked) {
  var _locked = [];
  for (var i = 0; i < locked.length; ++i) {
    _locked.push(new constraints.EqualsTo([locked[i]], locked[i].get()));
  }
  return _locked;
};

parametric.diagnose = function(sys) {
...
parametric.prepare = function(constrs, locked, aux, alg) {

  var simpleMode = true;
  if (!simpleMode) {
    var lockingConstrs = parametric.lock2Equals2(constrs, locked);
    Array.prototype.push.apply( constrs, lockingConstrs );
  }

  var sys = new parametric.System(constrs);

  parametric.wrapAux(constrs, aux);
  ...
  function solve(rough, alg) {
    ...
    switch (alg) {
      case 2:
        return parametric.solve_lm(sys, model, jacobian, rough);
      case 1:
      default:
  ...
  var systemSolver = {
    diagnose : function() {return parametric.diagnose(sys)},
...
parametric.solve_lm = function(sys, model, jacobian, rough) {
