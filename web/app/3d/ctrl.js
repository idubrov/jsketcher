...
function UI(app) {
  ...
  stlExport.root.click(function() {
    var allPolygons = utils.arrFlatten1L(app.findAllSolids().map(function (s) {
...
UI.createCutExtrudeWizard = function (isCut, app, face, alignComponent, initParams, overriding) {
  ...
  var polygons = craft.getSketchedPolygons3D(app, face);
  ...
  var wizard = new wizards.ExtrudeWizard(app.viewer, polygons);
  function onChange() {
...
UI.createPlaneWizard = function (app, alignComponent, initParams, overiding) {
  ...
  tk.add(folder, depth);
  var wizard = new wizards.PlaneWizard(app.viewer);
