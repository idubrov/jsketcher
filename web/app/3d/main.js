...
import { UI } from './ctrl';
import { Craft, craft, workbench } from '../workbench';
import { Matrix } from '../math/vector';
import math from '../math/math';
import { geom, utils } from '../engine';
...
App.prototype.sketchFace = function() {
  ...
  var paths = craft.reconstructSketchBounds(polyFace.solid.csg, polyFace);

  //polyFace.polygon.collectPaths(paths);
  var _3dTransformation = new Matrix().setBasis(polyFace.basis());
  ...
  function addArc(arc) {
    ...
    var c = Bumath.circleFromPoints(a, arc[mid], b);
    if (c == null) {
      addArcAsSegments(arc);
      return;
    }
  ...
  for (var i = 0; i < paths.length; i++) {
    ...
    utils.iteratePath(path, 0, function(a, b, ai, bi) {
      shift = bi;
      return sameSketchObject(a, b);
    });
    var currSko = null;
    var arc = null;
    utils.iteratePath(path, shift+1, function(a, b, ai, bi, iterNumber, path) {
