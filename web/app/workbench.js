...
import { utils, geom, Polygon } from './engine';
import struct from './3d/hashmap';
import { CSG } from 'openjscad-csg';

const workbench = {};

workbench.SketchConnection = function(a, b, sketchObject) {
  this.a = a;
  this.b = b;
  this.sketchObject = sketchObject;
};

workbench._SKETCH_OBJ_COUNTER = 0;
workbench.readSketchGeom = function(sketch) {
  function genId() {
    return workbench._SKETCH_OBJ_COUNTER++;
  }
    ...
    for (var l = 0; l < sketch.layers.length; ++l) {
      ...
      for (var i = 0; i < layer.data.length; ++i) {
        ...
        if (obj._class === 'TCAD.TWO.Segment') {
          var a = new Vector(obj.points[0][1][1], obj.points[0][2][1], 0);
          var b = new Vector(obj.points[1][1][1], obj.points[1][2][1], 0);
          out.connections.push(new workbench.SketchConnection(a, b, {_class : obj._class, id : genId()}));
        } else if (obj._class === 'TCAD.TWO.Arc') {
          var a = new Vector(obj.points[0][1][1], obj.points[0][2][1], 0);
          var b = new Vector(obj.points[1][1][1], obj.points[1][2][1], 0);
          var center = new Vector(obj.points[2][1][1], obj.points[2][2][1], 0);
          var approxArc = workbench.approxArc(a, b, center, 20);
          var data =  {_class : obj._class, id : genId()};
          for (var j = 0; j < approxArc.length - 1; j++) {
            out.connections.push(new workbench.SketchConnection(approxArc[j], approxArc[j+1], data));
          }
        } else if (obj._class === 'TCAD.TWO.Circle') {
          var center = new Vector(obj.c[1][1], obj.c[2][1], 0);
          var approxCircle = workbench.approxCircle(center, obj.r, 20);
          var data =  {_class : obj._class, id : genId()};
          var loop = [];
          var p, q, n = approxCircle.length;
          for (var p = n - 1, q = 0; q < n; p = q++) {
            loop.push(new workbench.SketchConnection(approxCircle[p], approxCircle[q], data));
          }
          out.loops.push(loop);
        }
      }
...
workbench.approxArc = function(ao, bo, c, resolution) {
  var a = ao.minus(c);
  var b = bo.minus(c);
  var points = [ao];
...
workbench.approxCircle = function(c, r, resolution) {
  var points = [];
...
workbench.serializeSolid = function(solid) {
  var data = {};
  data.faceCounter = geom.FACE_COUNTER; // FIXME: ???
...
craft.getSketchedPolygons3D = function(app, face) {
  ...
  var polygons2D = utils.sketchToPolygons(geom);
  ...
  for (var i = 0; i < polygons2D.length; i++) {
    ...
    if (depth == null) {
      var _3dTransformation = new Matrix().setBasis(face.basis());
...
craft.extrude = function(app, request) {
  var face = request.face;
  var sketchedPolygons = craft.getSketchedPolygons3D(app, face);
  if (sketchedPolygons == null) return null;

  var normal = utils.vec(face.csgGroup.plane.normal);
  var toMeldWith = [];
  for (var i = 0; i < sketchedPolygons.length; i++) {
    var extruded = geom.extrude(sketchedPolygons[i], normal, request.params.target, request.params.expansionFactor );
    toMeldWith = toMeldWith.concat(extruded);
  }

  var solid = request.solids[0];

  var meld = CSG.fromPolygons(craft._triangulateCSG(toMeldWith));
  if (solid.mergeable) {
    meld = solid.csg.union(meld);
  }

  face.csgGroup.shared.__tcad.faceId += '$';
  return [utils.createSolid(meld)];
};

craft._pointOnLine = function(p, a, b) {
  ...
  return apLength > 0 && apLength < abLength && utils.areEqual(abLength * apLength, dp, 1E-6);
};

craft.polygonsToSegments = function(polygons) {
  function selfIntersecting(a, b, c) {
    var f = craft._pointOnLine;
...
craft.reconstructSketchBounds = function(csg, face, strict) {
  strict = strict || false;
  ...
  for (var pi = 0; pi < polygons.length; pi++) {
    var poly = polygons[pi];
    if (utils.equal(poly.plane.normal.dot(plane.normal), 1)) {
      if (utils.equal(plane.w, poly.plane.w) && (!strict || !!poly.shared.__tcad && poly.shared.__tcad.faceId  === face.id)) {
    ...
    for(p = n - 1, q = 0; q < n; p = q ++) {
      var a = poly.vertices[p];
      var b = poly.vertices[q];
      var pointAOnPlane = utils.equal(plane.signedDistanceToPoint(a.pos), 0);
      if (!pointAOnPlane) continue;
      var pointBOnPlane = utils.equal(plane.signedDistanceToPoint(b.pos), 0);
      if (pointBOnPlane) {
  ...
  var outline = craft.findOutline(planePolygons);

  craft.pickUpCraftInfo(outline, outerEdges);

  return craft.segmentsToPaths(outline);
};

craft.pickUpCraftInfo = function(outline, outerEdges) {
  var eq = utils.strictEqual;
  for (var psi1 = 0; psi1 < outline.length; psi1++) {
    var s1 = outline[psi1];
    for (var psi2 = 0; psi2 < outerEdges.length; psi2++) {
      var s2 = outerEdges[psi2];
      if (utils.equal(Math.abs(s1[0].minus(s1[1]).unit().dot(s2[0].minus(s2[1]).unit())), 1) &&
          (eq(s1[0], s2[0]) || eq(s1[1], s2[1]) || eq(s1[0], s2[1]) || eq(s1[1], s2[0]) ||
          craft._pointOnLine(s1[0], s2[0], s2[1]) || craft._pointOnLine(s1[1], s2[0], s2[1]))) {
          s1[2] = s2[2];
      }
    }
  }
...
craft.getOutlineByCollision = function(segments, outerEdges) {
  var eq = utils.strictEqual;
  var outline = [];
 ...
 craft.findOutline = function(planePolygons, outer) {
  var segmentsByPolygon = craft.polygonsToSegments(planePolygons);
  //craft.simplifySegments(segmentsByPolygon);
  var planeSegments = utils.arrFlatten1L(segmentsByPolygon);
  //planeSegments = craft.removeSharedEdges(planeSegments);
  craft.removeTJoints(planeSegments);
  planeSegments = craft.removeSharedEdges(planeSegments);
  return planeSegments;
};

craft.removeSharedEdges = function(segments) {
...
craft.simplifySegments = function(polygonToSegments) {
    ...
    for (var pi2 = 0; pi2 < polygonToSegments.length; ++pi2) {
      ...
      for (var si1 = 0; si1 < polygon1.length; ++si1) {
        ...
        for (var si2 = 0; si2 < polygon2.length; ++si2) {
          ...
          if (craft._pointOnLine(point, seg1[0], seg1[1])) {
...
craft._closeFactorToLine = function(p, seg1, seg2) {
...
craft.removeTJoints = function(segments) {
  var pointIndex = struct.hashTable.forVector3d();
  ...
  for (var pi1 = 0; pi1 < points.length; ++pi1) {
    ...
    for (var pi2 = 0; pi2 < segments.length; ++pi2) {
      ...
      var factor = craft._closeFactorToLine(point, seg[0], seg[1]);
...
craft.deleteRedundantPoints = function(path) {
  var cleanedPath = [];
...
craft.segmentsToPaths = function(segments) {

  var veq = struct.hashTable.vectorEquals;

  var paths = [];
  var index = struct.hashTable.forVector3d();
  var csgIndex = struct.hashTable.forEdge();
  ...
  for (var i = 0; i < paths.length; i++) {
    ...
    utils.iteratePath(path, 0, function (a, b) {
    ...
    path = craft.deleteRedundantPoints(path);
    if (path.length > 2) {
      filteredPaths.push({
        vertices: path
      });
...
craft._triangulateCSG = function(polygons) {
  function csgVec(v) {
  ...
  for (var ei = 0; ei < polygons.length; ++ei) {
    ...
    var refs = geom.triangulate(points, poly.plane.normal);
...
craft.splitTwoSegments = function (a, b) {
...
craft.attract = function(vectors, precision) {
  var eq = utils.areEqual();
...
craft.recoverySketchInfo = function(polygons) {
  var nonStructuralGons = [];
  var sketchEdges = struct.hashTable.forDoubleArray();
...
craft.reconstruct = function (cut) {
  function pInP(p1, p2) {
    var notEqPoints = [];
    ...
    for (var i = 0; i < notEqPoints.length; ++i) {
      var v = notEqPoints[i];
      if (!utils.isPointInsidePolygon(v, p2)) {
        return false;
      }
    }
  ...
  function sortPaths(paths3D) {
    ...
    var transforms = struct.hashTable.forVector3d();

    var paths = paths3D.map(function(path) {
      var tr = transforms.get(path.normal);
      if (tr === null) {
        var _3dTransformation = new Matrix().setBasis(geom.someBasis(path.vertices, path.normal));
  ...
  for (var tag in byShared) {
    var merged = craft._mergeCSGPolygons(byShared[tag], allPoints);
    var sorted = sortPaths(merged);
    result.push.apply(result, sorted.map(function(path) {
        var p = new Polygon(path.vertices, path.holes.map(function (path) {
  ...
  craft.recoverySketchInfo(result);
  return result;
};

craft.collectFaces = function(solids) {
  var faces = [];
  for (var i = 0; i < solids.length; i++) {
     utils.addAll(faces, solids[i].polyFaces);
  }
  return faces;
};

craft.collectCSGPolygons = function(faces) {
  var out = [];
  for (var fi = 0; fi < faces.length; fi++) {
    var face = faces[fi];
    utils.addAll(out, face.csgGroup.toCSGPolygons());
  }
  return out;
};

craft.toGroups = function(csgPolygons) {
  ...
  for (var tag in byShared) {
    ...
    var simplePolygons = groupedPolygons.map(function (p) {
      ...
      return new TCAD.SimplePolygon(vertices, normal); // FIXME: ???
    });
    var csgGroup = new TCAD.CSGGroup(simplePolygons, normal, plane.w); // FIXME: ???
...
craft.cut = function(app, request) {
  var face = request.face;
  var sketchedPolygons = craft.getSketchedPolygons3D(app, face);
  if (sketchedPolygons == null) return null;
  ...
  var cutterCSG = CSG.fromPolygons(craft._triangulateCSG(cutter));
  ...
  for (var si = 0; si < request.solids.length; si++) {
    ...
    var solidMesh = utils.createSolid(cut);
...
Craft.prototype.reset = function(modifications) {
  ...
  for (var i = 0; i < modifications.length; i++) {
    var request = craft.materialize(this.app.indexEntities(), modifications[i]);
  ...
  for (var prop in request) {
    if (request.hasOwnProperty(prop)) {
      ...
      if (prop == 'solids') {
        ...
        detachedConfig[prop] = craft.detach(value);
      } else {
...
craft.materialize = function(index, detachedConfig) {
  var request = {};
  ...
  for (var prop in detachedConfig) {
    if (detachedConfig.hasOwnProperty(prop)) {
      ...
      if (prop == 'solids') {
        ...
        request[prop] = craft.materialize(index, value);
      } else {
        request[prop] = value;
      }
    }
...
Craft.prototype.modify = function(request, overriding) {
  this.modifyInternal(request);
  var detachedRequest = craft.detach(request);
...
craft.OPS = {
  ...
  PLANE : function(app, request) {
    return [utils.createPlane(request.params.basis, request.params.depth)];
  },
  BOX : function(app, request) {
    return [utils.createCSGBox(request.size)];
  }
};

export { workbench, craft, Craft };
